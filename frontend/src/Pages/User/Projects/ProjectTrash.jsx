...
const ProjectTrash = () => {
  ...
  const restoreProject = async (projectId) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API}/api/project/restore/${projectId}`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${auth.token}` },
      });
  ...
  const deleteProject = async (projectId) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API}/api/project/pdeleteProject/${projectId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${auth.token}` },
      });
      const result = await response.json();
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          <div className="max-w-4xl mx-auto">
            <h1 className="text-2xl font-bold text-gray-800 mb-2">Project Trash</h1>
            <p className="text-sm text-red-600 mb-6">
              Projects in the trash will be permanently deleted after 30 days.
            </p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
            <h2 className="text-lg font-semibold text-gray-800 mb-4">
              {confirmDialog.action === 'restore' ? 'Restore Project' : 'Delete Project Permanently'}
            </h2>
