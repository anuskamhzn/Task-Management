...
export function Metrics({ refreshTrigger }) {
  ...
  return (
    <div className="p-4 sm:p-6 bg-white rounded-lg shadow-lg border">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6">
        <h2 className="text-lg sm:text-xl font-semibold text-gray-800 flex items-center mb-4 sm:mb-0">
          <svg
            className="w-5 h-5 sm:w-6 sm:h-6 mr-2 text-purple-700"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all text-sm sm:text-base ${
              activeTab === "task"
      ...
      {loading ? (
        <div className="flex justify-center items-center py-8">
          <svg
            className="animate-spin h-6 w-6 sm:h-8 sm:w-8 text-purple-700"
        ...
        <div className="text-red-600 py-4 flex items-center justify-center text-sm sm:text-base">
          <svg
        ...
        <div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
          {metrics.map((metric) => (
            <div
              key={metric.title}
              className={`rounded-lg border p-4 sm:p-6 shadow-sm hover:shadow-md transition-all ${metric.color}`}
            >
              <div className="flex justify-between items-start">
                <h3 className="text-xs sm:text-sm font-medium text-gray-600">{metric.title}</h3>
                {renderIcon(metric.icon)}
              </div>
              <p className="mt-3 sm:mt-4 text-2xl sm:text-3xl font-bold">{metric.value}</p>
            </div>
          ))}
        </div>
      )}
